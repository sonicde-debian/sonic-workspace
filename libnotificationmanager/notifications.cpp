...
{
    const QModelIndex sourceIndex = Utils::mapToModel(idx, nullptr);
    auto *sourceModel = const_cast<QAbstractItemModel *>(sourceIndex.model());

    if (auto *notificationModel = qobject_cast<AbstractNotificationsModel *>(sourceModel)) {
        const auto notificationId = Private::notificationId(idx);
        notificationModel->expire(notificationId);
    } else if (auto *jobsModel = qobject_cast<JobsModel *>(sourceModel)) {
        jobsModel->expire(sourceIndex);
    }
}
...
void Notifications::close(const QModelIndex &idx)
{
    ...
    if (auto *notificationModel = qobject_cast<AbstractNotificationsModel *>(sourceModel)) {
        const auto notificationId = Private::notificationId(idx);
        notificationModel->close(notificationId);
    } else if (auto *jobsModel = qobject_cast<JobsModel *>(sourceModel)) {
        jobsModel->close(sourceIndex);
    }
}
...
void Notifications::configure(const QModelIndex &idx)
{
    ...
    if (auto *notificationsModel = qobject_cast<NotificationsModel *>(sourceModel)) {
        notificationsModel->configure(notificationId);
    } else if (auto *portalNotificationsModel = qobject_cast<PortalNotificationsModel *>(sourceModel)) {
        portalNotificationsModel->configure(notificationId);
    }
}
...
void Notifications::invokeDefaultAction(const QModelIndex &idx, InvokeBehavior behavior)
{
    ...
    if (auto *notificationModel = qobject_cast<AbstractNotificationsModel *>(sourceModel)) {
        const auto notificationId = Private::notificationId(idx);
        notificationModel->invokeDefaultAction(notificationId, behavior);
    }
}
...
void Notifications::invokeAction(const QModelIndex &idx, const QString &actionId, InvokeBehavior behavior)
{
    ...
    if (auto *notificationModel = qobject_cast<AbstractNotificationsModel *>(sourceModel)) {
        const auto notificationId = Private::notificationId(idx);
        notificationModel->invokeAction(notificationId, actionId, behavior);
    }
}
...
void Notifications::reply(const QModelIndex &idx, const QString &text, InvokeBehavior behavior)
{
    ...
    if (auto *notificationModel = qobject_cast<AbstractNotificationsModel *>(sourceModel)) {
        const auto notificationId = Private::notificationId(idx);
        notificationModel->reply(notificationId, text, behavior);
    }
}
