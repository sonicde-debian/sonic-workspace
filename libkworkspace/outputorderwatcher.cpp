...
class X11OutputOrderWatcher : public OutputOrderWatcher,
...
private:
  ...
  QTimer *m_delayTimer =
      nullptr; // This is just to simulate the protocol that will be guaranteed
               // to always arrive after screens additions and removals
  // Xrandr
...
OutputOrderWatcher::OutputOrderWatcher(QObject *parent) : QObject(parent) {
...
void OutputOrderWatcher::useFallback(bool fallback, const char *reason) {
  ...
  if (fallback) {
    if (reason) {
      qCritical() << "OutputOrderWatcher may not work as expected. Reason:"
                  << reason;
    }
    connect(qGuiApp, &QGuiApplication::primaryScreenChanged, this,
            &OutputOrderWatcher::refresh, Qt::UniqueConnection);
    refresh();
  }
}
...
void OutputOrderWatcher::refresh() {
  ...
  QStringList pendingOutputOrder = qGuiApp->screens() |
                                   std::views::transform(&QScreen::name) |
...
QStringList OutputOrderWatcher::outputOrder() const { return m_outputOrder; }
...
X11OutputOrderWatcher::X11OutputOrderWatcher(QObject *parent)
  ...
  // This timer is used to signal only when a qscreen for every output is
  // already created, perhaps by monitoring screenadded/screenremoved and
  // tracking the outputs still missing
  m_delayTimer = new QTimer(this);
  m_delayTimer->setSingleShot(true);
  m_delayTimer->setInterval(0);
  connect(m_delayTimer, &QTimer::timeout, this, [this]() { refresh(); });
  ...
  const xcb_query_extension_reply_t *reply =
      xcb_get_extension_data(m_x11Interface->connection(), &xcb_randr_id);
  if (!reply ||
      !reply->present) { // SENTRY PLASMA-WORKSPACE-1MMC: XRandr extension is
                         // not initialized when using vncserver
  ...
  xcb_intern_atom_cookie_t atomCookie = xcb_intern_atom_unchecked(
      m_x11Interface->connection(), false,
      std::char_traits<char>::length(effectName), effectName);
  xcb_intern_atom_reply_t *atom(
      xcb_intern_atom_reply(m_x11Interface->connection(), atomCookie, nullptr));
  if (!atom) {
    useFallback(true);
    return;
  }
...
void X11OutputOrderWatcher::refresh() {
  ...
  ScopedPointer<xcb_randr_get_screen_resources_current_reply_t> reply(
      xcb_randr_get_screen_resources_current_reply(
          ...
          xcb_randr_get_screen_resources_current(
              m_x11Interface->connection(),
              DefaultRootWindow(m_x11Interface->display())),
          NULL));
  ...
  xcb_randr_output_t *randr_outputs =
      xcb_randr_get_screen_resources_current_outputs(reply.data());

  for (int i = 0; i < len; i++) {
    ScopedPointer<xcb_randr_get_output_info_reply_t> output(
        xcb_randr_get_output_info_reply(
            m_x11Interface->connection(),
            xcb_randr_get_output_info(m_x11Interface->connection(),
                                      randr_outputs[i], timestamp),
            NULL));
    ...
    auto orderCookie = xcb_randr_get_output_property(
        m_x11Interface->connection(), randr_outputs[i], m_kdeScreenAtom,
        XCB_ATOM_ANY, 0, 100, false, false);
    ScopedPointer<xcb_randr_get_output_property_reply_t> orderReply(
        xcb_randr_get_output_property_reply(m_x11Interface->connection(),
                                            orderCookie, nullptr));
    // If there is even a single screen without _KDE_SCREEN_INDEX info, fall
    ...
    if (!(orderReply->type == XCB_ATOM_INTEGER && orderReply->format == 32 &&
          orderReply->num_items == 1)) {
      useFallback(true);
      return;
    }

    const uint32_t order =
        *xcb_randr_get_output_property_data(orderReply.data());

    if (order > 0) { // 0 is the special case for disabled, so we ignore it
      orderMap.emplace_back(
          order, QString::fromUtf8(
  ...
  std::ranges::transform(screens, std::back_inserter(screenNames),
                         [](const QScreen *screen) { return screen->name(); });
  const bool isScreenPresent =
      std::ranges::all_of(orderMap, [&screenNames](const auto &pr) {
        return std::ranges::find(screenNames, std::get<QString>(pr)) !=
               screenNames.end();
      });
  ...
  const auto getAllValues =
      [](const QList<std::pair<uint, QString>> &orderMap) -> QList<QString> {
    QList<QString> values;
    values.reserve(orderMap.size());
    std::transform(orderMap.cbegin(), orderMap.cend(),
                   std::back_inserter(values),
                   [](const auto &pair) { return pair.second; });
    return values;
  };
  if (const auto pendingOutputs = getAllValues(orderMap);
      pendingOutputs != m_outputOrder) {
    m_outputOrder = pendingOutputs;
#else
  if (const auto pendingOutputs = std::views::values(std::as_const(orderMap));
      !std::ranges::equal(pendingOutputs, std::as_const(m_outputOrder))) {
