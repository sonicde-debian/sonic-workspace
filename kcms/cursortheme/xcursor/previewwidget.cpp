...
constexpr const char *const cursor_names[] = {
    "left_ptr", "left_ptr_watch", "wait",       "pointer", "help",
    "ibeam",    "size_all",       "size_fdiag", "cross",   "split_h",
    "size_ver", "size_hor",       "size_bdiag", "split_v",
};
...
PreviewWidget::PreviewWidget(QQuickItem *parent)
    : QQuickPaintedItem(parent), m_currentIndex(-1), m_currentSize(0) {
  setAcceptHoverEvents(true);
  current = nullptr;
  connect(&m_animationTimer, &QTimer::timeout, this, [this] {
    Q_ASSERT(current);
    const auto &frame = current->images().at(nextAnimationFrame);
    setCursor(QCursor(QPixmap::fromImage(frame.image), frame.hotspot.x(),
                      frame.hotspot.y()));
...
SortProxyModel *PreviewWidget::themeModel() { return m_themeModel; }
...
int PreviewWidget::currentIndex() const { return m_currentIndex; }
...
int PreviewWidget::currentSize() const { return m_currentSize; }

int PreviewWidget::maximumCount() const { return m_maximumCount; }
...
int PreviewWidget::padding() const { return m_padding; }
...
int PreviewWidget::spacing() const { return m_spacing; }
...
void PreviewWidget::refresh() {
  ...
  const CursorTheme *theme =
      m_themeModel->theme(m_themeModel->index(m_currentIndex, 0));
...
void PreviewWidget::setTheme(const CursorTheme *theme, const int size) {
  ...
  if (theme) {
    ...
    for (int i = 0; i < numCursors; i++) {
      list << new PreviewCursor(theme, QString::fromLatin1(cursor_names[i]),
                                size);
    }
...
void PreviewWidget::paint(QPainter *painter) {
  ...
  for (const auto *c : std::as_const(list)) {
    const QImage image =
        !c->images().empty() ? c->images().front().image : QImage();
    if (image.isNull()) {
      continue;
    }
...
auto it = std::ranges::find_if(
    list, [this, e, devicePixelRatio](const PreviewCursor *c) {
      // Increase hit area.
      const int padding = m_spacing / 2;
      const auto adjustedRect =
          c->rect().adjusted(-padding, -padding, +padding, +padding);
...
if (images.size() == 1) {
  ...
  setCursor(QCursor(QPixmap::fromImage(image.image), image.hotspot.x(),
                    image.hotspot.y()));
  return;
}
...
void PreviewWidget::geometryChange(const QRectF &newGeometry,
