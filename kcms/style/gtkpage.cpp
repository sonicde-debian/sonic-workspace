...
void GtkPage::installGtkThemeFromFile(const QUrl &fileUrl)
{
    ...
    if (!themeArchive.open(QIODevice::ReadOnly)) {
        qWarning() << "Failed to open archive file:" << themeArchive.errorString();
    }
    ...
    if (possibleThemeDirectory->isDirectory()) {
        ...
        if (!archiveSubitems.contains(u"gtk-2.0") && archiveSubitems.indexOf(QRegularExpression(QStringLiteral("gtk-3.*"))) == -1) {
            showError();
            return;
        }
    } else {
