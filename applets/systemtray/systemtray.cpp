...
SystemTray::SystemTray(QObject *parent, const KPluginMetaData &data,
                       const QVariantList &args)
    : Plasma::Containment(parent, data, args) {
  setHasConfigurationInterface(true);
  setContainmentDisplayHints(Plasma::Types::ContainmentDrawsPlasmoidHeading |
                             Plasma::Types::ContainmentForcesSquarePlasmoids);
}

SystemTray::~SystemTray() {
  // When the applet is about to be deleted, delete now to avoid calling
  // loadConfig()
  delete m_settings;
}
...
void SystemTray::init() {
  ...
  // Applet to the Containment even if SystemTray is of type Containment, it is
  // de facto Applet and should act like one
  connect(this, &Containment::appletAdded, this,
...
void SystemTray::initSettingsAndRegistry() {
  if (!m_settings) {
    ...
    connect(m_settings, &SystemTraySettings::enabledPluginsChanged, this,
            &SystemTray::onEnabledAppletsChanged);
  }

  if (!m_plasmoidRegistry) {
    m_plasmoidRegistry = new PlasmoidRegistry(m_settings, this);
    connect(m_plasmoidRegistry, &PlasmoidRegistry::plasmoidEnabled, this,
            &SystemTray::startApplet);
    connect(m_plasmoidRegistry, &PlasmoidRegistry::plasmoidStopped, this,
            &SystemTray::stopApplet);
  }
}
...
void SystemTray::migrateFromSystrayContainer() {
  ...
  // NOTE: this function is called from the constructor, so we can't use
  // config() yet
  ...
  const uint oldSystrayId =
      oldAppletCg.readEntry(QStringLiteral("SystrayContainmentId"), 0);
  ...
  while (!queue.isEmpty()) {
    ...
    for (auto it = entries.constBegin(); it != entries.constEnd(); ++it) {
      currentDest.writeEntry(it.key(),
                             currentSource.readEntry(it.key(), QString()));
    }
  ...
  // To make sure the old containment is really gone from the config first
  // delete then remove the group, as destroy() might fail
...
void SystemTray::restoreContents(KConfigGroup &group) {
  if (!isContainment()) {
    qCWarning(SYSTEM_TRAY)
        << "Loaded as an applet, this shouldn't have happened";
    return;
  }

  KConfigGroup shortcutConfig(&group, u"Shortcuts"_s);
  QString shortcutText =
      shortcutConfig.readEntryUntranslated("global", QString());
...
void SystemTray::showPlasmoidMenu(QQuickItem *appletInterface, int x, int y) {
  ...
  // this is a workaround where Qt will fail to realize a mouse has been
  // released

  // this happens if a window which does not accept focus spawns a new window
  // that takes focus and X grab whilst the mouse is depressed
  // https://bugreports.qt.io/browse/QTBUG-59044
  // this causes the next click to go missing

  // by releasing manually we avoid that situation
  auto ungrabMouseHack = [appletInterface]() {
    if (appletInterface->window() &&
        appletInterface->window()->mouseGrabberItem()) {
  ...
  if (QScreen *screen = appletInterface->window()->screen()) {
    ...
    pos = QPoint(
        qBound(geo.left(), (int)pos.x(), geo.right() - desktopMenu->width()), //
...
QPointF SystemTray::popupPosition(QQuickItem *visualParent, int x, int y) {
  ...
  if (window && window->screen()) {
    pos = window->mapToGlobal(pos.toPoint());
    {
      ...
      const QPointF nativeGlobalPosOnCurrentScreen =
          (pos - geometry.topLeft()) * devicePixelRatio;
...
SystemTrayModel *SystemTray::systemTrayModel() {
  if (!m_systemTrayModel) {
    ...
    m_plasmoidModel =
        new PlasmoidModel(m_settings, m_plasmoidRegistry, m_systemTrayModel);
    connect(this, &SystemTray::appletAdded, m_plasmoidModel,
            &PlasmoidModel::addApplet);
    connect(this, &SystemTray::appletRemoved, m_plasmoidModel,
            &PlasmoidModel::removeApplet);
    for (auto applet : applets()) {
      m_plasmoidModel->addApplet(applet);
    }

    m_statusNotifierModel =
        new StatusNotifierModel(m_settings, m_systemTrayModel);
...
QAbstractItemModel *SystemTray::sortedSystemTrayModel() {
  if (!m_sortedSystemTrayModel) {
    m_sortedSystemTrayModel = new SortedSystemTrayModel(
        SortedSystemTrayModel::SortingType::SystemTray, this);
...
QAbstractItemModel *SystemTray::configSystemTrayModel() {
  if (!m_configSystemTrayModel) {
    m_configSystemTrayModel = new SortedSystemTrayModel(
        SortedSystemTrayModel::SortingType::ConfigurationPage, this);
...
void SystemTray::startApplet(const QString &pluginId) {
  ...
  if (m_configGroupIds.contains(pluginId)) {
    Applet *applet = Plasma::PluginLoader::self()->loadApplet(
        pluginId, m_configGroupIds.value(pluginId), QVariantList());
    // this should never happen unless explicitly wrong config is hand-written
    // or
    ...
    Applet *applet = createApplet(
        pluginId, QVariantList() << u"org.kde.plasma:force-create"_s);
    if (applet) {
...
void SystemTray::stopApplet(const QString &pluginId) {
  ...
  for (Plasma::Applet *applet : appletsList) {
    if (applet->pluginMetaData().isValid() &&
        pluginId == applet->pluginMetaData().pluginId()) {
      delete applet;
    }
  }
}
...
void SystemTray::activate(const QString &service, QPoint pos,
                          QQuickItem *statusNotifierIcon) {
  const auto source = StatusNotifierItemHost::self()->itemForService(service);

  if (!source) {
    qCWarning(SYSTEM_TRAY) << "activate: Could not find item for service"
                           << service;
    return;
  }

  connect(
      source, &StatusNotifierItemSource::activateResult, this,
...
void SystemTray::secondaryActivate(const QString &service, QPoint pos) {
  ...
  if (!source) {
    qCWarning(SYSTEM_TRAY)
        << "secondaryActivate: Could not find item for service" << service;
    return;
  }
...
void SystemTray::openContextMenu(const QString &service, QPoint pos,
                                 QQuickItem *statusNotifierIcon) {
  const auto source = StatusNotifierItemHost::self()->itemForService(service);

  if (!source) {
    qCWarning(SYSTEM_TRAY) << "openContextMenu: Could not find item for service"
                           << service;
    return;
  }

  connect(
      source, &StatusNotifierItemSource::contextMenuReady, this,
      [this, statusNotifierIcon, pos](QMenu *menu) {
        if (menu && !menu->isEmpty()) {
          ...
          // try tofind the icon screen coordinates, and adjust the position as
          // a poor man's popupPosition

          QRect screenItemRect(
              statusNotifierIcon->mapToScene(QPointF(0, 0)).toPoint(),
              QSize(statusNotifierIcon->width(), statusNotifierIcon->height()));

          if (statusNotifierIcon->window()) {
            screenItemRect.moveTopLeft(
                statusNotifierIcon->window()->mapToGlobal(
                    screenItemRect.topLeft()));
          }
          ...
          default:
            x = screenItemRect.left();
            if (screenItemRect.top() - menu->height() >=
                statusNotifierIcon->window()->screen()->geometry().top()) {
              y = screenItemRect.top() - menu->height();
            } else {
              y = screenItemRect.bottom();
            }
          ...
          menu->winId();
          menu->windowHandle()->setTransientParent(
              statusNotifierIcon->window());
...
void SystemTray::scroll(const QString &service, int delta,
                        const QString &direction) {
  const auto source = StatusNotifierItemHost::self()->itemForService(service);

  if (!source) {
    qCWarning(SYSTEM_TRAY) << "scroll: Could not find item for service"
                           << service;
    return;
  }
